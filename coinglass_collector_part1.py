"""
Coinglass Data Collector - Part 1 of 4
...
PAIRS = [
    "BTC", "SOL", "XRP", "ADA", "AVAX", "DOT", "NEAR", "BCH"
]
...
def save_to_excel(pair, oi_data, curr_data, vol_data):
    ...
    # Clean up stale lock file if exists and is old
    if os.path.exists(LOCK_FILE):
        try:
            lock_age = time.time() - os.path.getmtime(LOCK_FILE)
            if lock_age > 300:  # 5 minutes old
                os.remove(LOCK_FILE)
...
async def main():
    print(f"\n🚀 Coinglass Data Collector - PART 1")
    print(f"⏰ Time: {datetime.now().strftime('%Y-%m-%d %I:%M:%S %p')}")
    print(f"📈 Pairs: {len(PAIRS)} (Part 1 of 4)")
